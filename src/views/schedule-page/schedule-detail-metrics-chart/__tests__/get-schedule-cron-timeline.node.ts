...
describe('schedule cron timeline', () => {
  ...
  it('returns the latest requested occurrences without crossing the start', () => {
    expect(
      getLatestExpectedScheduleTimesMs({
        cronExpression: '0 * * * *',
        anchorMs: Date.parse('2026-07-26T12:00:00Z'),
        startMs: Date.parse('2026-07-26T10:00:00Z'),
        count: 20,
      })
    ).toEqual([
  ...
  it('calculates the following interval for irregular schedules', () => {
    expect(
      getScheduleIntervalAfterMs({
        cronExpression: '0 0 1 * *',
        occurrenceMs: Date.parse('2026-02-01T00:00:00Z'),
      })
    ).toBe(28 * 24 * HOUR_MS);
  });
  ...
  it('uses creation, update, retention, and spec bounds', () => {
    ...
    expect(
      ...
      navigationStartMs: Date.parse('2026-07-19T12:00:00Z'),
